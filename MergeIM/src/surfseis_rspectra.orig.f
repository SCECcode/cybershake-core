...
c
c
      subroutine spectrad(nx,ny,npts,dt,cin_units,cout_units,
     + 		cout_choice, cout_numperiods,filter_highHZ,
     +		capply_byteswap,ofile,seis)
	implicit none
	integer		maxperiod,maxdamp,maxnpt
     ...
     , a(maxnpt),time(maxnpt),b(maxperiod),z(3), peat(112), scec(45)
	real outarray(maxperiod)
c
CME   character title*75,head*75,fdesc*20,ifile*40,ofile*40,rfile*40,
CME  , word*75,fmt*40
	character title*75,head*75,fdesc*20,ofile*256,rfile*120,
     ,	   word*75,fmt*40
...
c               ichk = setparmf("surfseis_rspectra_period",period_out)
	...
	call getparms(ofile,nx,ny,npts,dt, fac,ndamp,damp,maxdamp,
...
c==================================================================
	subroutine getparms(ofile,nx,ny,npts,dt, fac,ndamp,damp,maxdamp,
     +		cperiod, capply_demean,capply_filter,capply_vel2accel,
     +		filter_lowHZ,filter_highHZ,period_out,capply_byteswap,
     +		cout_choice,cin_units,cout_units,jout_format,jout_numperiods)
	implicit none
...
c		if(dt .GT. 0)filter_highHZ = 1/(2.*dt)
