...
int run_synth(task_info* t_info, int* proc_points, int num_sgt_handlers, char stat[64], float slat, float slon, int run_id, float det_max_freq, float stoch_max_freq, int run_PSA, int run_rotd, int run_duration, int my_id) {
    ...
    int binarySRF = 0;
    read_srf(&srf,t_info->task->rupture_filename,binarySRF);
    //printf("srf @ %p, srf.srf_apnts@ %p, srf.srf_apnts.apntvals @ %p\n", &srf, srf.srf_apnts, srf.srf_apnts.apntvals);

    int num_srf_pts = srf.srf_apnts.np;
    ...
    for(ip=0;ip<num_srf_pts;ip++) {
    	elon = srf.srf_apnts.apntvals[ip].lon;
        elat = srf.srf_apnts.apntvals[ip].lat;
        edep = srf.srf_apnts.apntvals[ip].dep;
	...
	if (debug) write_log("Starting jbsim3d_synth.");
        //printf("Calling jbsim3d: srf @ %p, srf.srf_apnts@ %p, srf.srf_apnts.apntvals @ %p\n", &srf, srf.srf_apnts, srf.srf_apnts.apntvals);

	float** original_seis = jbsim3d_synth(&seis, &header, stat, slon, slat, ntout, seis_filename,
			&srf, t_info->sgtfilepar, sgtparms, *(t_info->sgtmast), t_info->sgtindx, geop, indx_master, nm, sgts_by_handler,
