...
//printf("Searched for %ld, best match was %ld.\n", eqindx->indx, sgtindx[ip].indx);
...
for(ig=0;ig<nsgt;ig++)
   ...
   for(it=tapst+1;it<sgtheadptr->nt;it++)
      ...
      //printf("gf0[%d] = %e, gf1[%d] = %e, gf2[%d] = %e\n", it, gf0[it], it, gf1[it], it, gf2[it]);
      gf0[it] = fac*gf0[it];
      gf1[it] = fac*gf1[it];
      gf2[it] = fac*gf2[it];
      }
...
/* apply time shift */
   ...
   it0 = sgtheadptr->nt + itshift;
	...
	//printf("tsh=%f, itshift=%d, sgtheadptr->dt=%f, sgtheadptr->nt=%d, it0=%f\n", tsh, itshift, sgtheadptr->dt, sgtheadptr->nt, it0);
...
for(im=0;im<mp.nmech;im++)
   ...
   for(ig=0;ig<(sgtpar->nsgt);ig++)
      ...
      if(sgtheadptr->lam > 0.0) /* strain green tensor */
         {
...
//void mech_sgt_d(float *gfmech,float *sgtbuf,struct sgtheader *sgthead,struct sgtparams *sgtpar,int nts,struct mechparam mp,double *d_scl)
{
struct sgtheader *sgtheadptr;
float *sgtbufptr;
float *zdd, *rdd, *zds, *rds, *tds, *zss, *rss, *tss;
...
float us, ud, ux, uy, uz;
int it, ig, im;
double d_sum, d_xamp, d_yamp, d_zamp;
...
for(im=0;im<mp.nmech;im++)
   ...
   for(ig=0;ig<(sgtpar->nsgt);ig++)
      ...
      d_sum = d_sum + (*d_scl)*(double)(sgtheadptr->mu)*(double)(sgtpar->wt[ig]);

      d_xamp = 0.0;
      if(sgtheadptr->xmom > 0.0)
         d_xamp = (*d_scl)*(double)(sgtpar->wt[ig])/(double)(sgtheadptr->xmom);

      d_yamp = 0.0;
      if(sgtheadptr->ymom > 0.0)
         d_yamp = (*d_scl)*(double)(sgtpar->wt[ig])/(double)(sgtheadptr->ymom);

      d_zamp = 0.0;
      if(sgtheadptr->zmom > 0.0)
         d_zamp = (*d_scl)*(double)(sgtpar->wt[ig])/(double)(sgtheadptr->zmom);

      for(it=0;it<sgtheadptr->nt;it++)
         {
         sx = d_xamp*(axx[it]*mxx + ayy[it]*myy + azz[it]*mzz
               + axy[it]*mxy + axz[it]*mxz + ayz[it]*myz);

         sy = d_yamp*(bxx[it]*mxx + byy[it]*myy + bzz[it]*mzz
               + bxy[it]*mxy + bxz[it]*mxz + byz[it]*myz);

         sz = d_zamp*(cxx[it]*mxx + cyy[it]*myy + czz[it]*mzz
               + cxy[it]*mxy + cxz[it]*mxz + cyz[it]*myz);
...
*d_scl = d_sum;  /* scl now contains the moment released for this point source */
}
