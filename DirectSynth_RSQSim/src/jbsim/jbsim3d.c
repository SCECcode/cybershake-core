...
float** jbsim3d_synth(float*** seis_return, struct seisheader* header, char stat[], float slon, float slat,
		int ntout, char seis_file[], struct standrupformat* passed_srf, struct sgtfileparams* sfp,
...
for (i=0; i<num_rup_vars; i++) {
	...
	apv_off[i] = 0;
	//fprintf(stderr, "SRF structure %d reports np=%d\n", i, apnts_ptr[i]->np);
	//fprintf(stderr, "apval_ptr[0][0] = %p\n", apval_ptr[0][0]);
}
