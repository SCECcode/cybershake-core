...
C       call openlog(1)
     ...
     +       nedy2,nskpy2,nbgz2,nedz2,nskpz2,ntiskp2,nve,
     +       mu_ss,mu_dd,fl,
...
C     =======================================
     ...
     +              wu2ed,ed2wu,wd2eu,eu2wd)
           ...
           ! ------------------------- 
           call addsrc(i,dh,dt,nst,npsrc,read_step,ifault,rank,igreen)
        endif
      endif
...
c     if(fsf==1) call fstr(nxt,nyt,nzt)
     ...
     +                 nsrc,nst,npsrc,nz,i/read_step +1,read_step)
               ...
               call addsrc(i+1,dh,dt,nst,npsrc,read_step,ifault,rank,igreen)
            endif               !end of (ifault==)
...
cSGT -------start---------------------------------------
        if (igreen .ne. -1 .and. mod(i,ntiskp2).eq.0) then
...
!********************   MPI Output   **************************
     ...
     +                              int(int(nt/write_step)*write_step/ntiskp)
...
!                                write (*,*), 'k=', k,'offset=',sgt_offset
                                sgt_offset=sgt_offset+(int(i/ntiskp/write_step)-1)*floatsize*write_step
!                                write (*,*), 'i/ntiskp/write_step=', int(i/ntiskp/write_step),'offset=',sgt_offset
