...
!!    Read axx,...axy from corresponding splitted sub-grid fault file
     ...
     +         recl=bsize*npsrc*read_step*3, status='old')
      ...
      subroutine addsrc(i,dx,dt,nst,npsrc,read_step,ifault,rank,igreen,nzt)

CCC   ROUTINE TO ADD STAGGERED-GRID SOURCE TO STRESS TENSOR
      use parstat
      implicit none

      integer :: i,nst,npsrc,read_step,ifault,rank,igreen,nzt
      real :: dt,dx,vtst,vtst1
      integer :: j,k
...
C       MARCH OVER NUMBER OF FAULT NODES  IN PROC
     ...
     +    w1(psrc(j,1),psrc(j,2),psrc(j,3)) + azz(j,k)*vtst/d1(psrc(j,1),psrc(j,2),psrc(j,3))
          ...
          elseif (igreen.eq.4) then

          xz(psrc(j,1),psrc(j,2),nzt) = axx(j,k)*vtst1
          xz(psrc(j,1),psrc(j,2),nzt+1) = 2.*axx(j,k)*vtst1- 
     +      xz(psrc(j,1),psrc(j,2),nzt-1)
          xz(psrc(j,1),psrc(j,2),nzt+2) = 2.*axx(j,k)*vtst1- 
     +      xz(psrc(j,1),psrc(j,2),nzt-2)
           
          elseif (igreen.eq.5) then

          yz(psrc(j,1),psrc(j,2),nzt) = ayy(j,k)*vtst1
          yz(psrc(j,1),psrc(j,2),nzt+1) = 2.*ayy(j,k)*vtst1- 
     +      yz(psrc(j,1),psrc(j,2),nzt-1)
          yz(psrc(j,1),psrc(j,2),nzt+2) = 2.*ayy(j,k)*vtst1- 
     +      yz(psrc(j,1),psrc(j,2),nzt-2)

          elseif (igreen.eq.6) then

          zz(psrc(j,1),psrc(j,2),nzt+1) = 2.*azz(j,k)*vtst1- 
     +      zz(psrc(j,1),psrc(j,2),nzt)
          zz(psrc(j,1),psrc(j,2),nzt+2) = 2.*azz(j,k)*vtst1- 
     +      zz(psrc(j,1),psrc(j,2),nzt-1)
