...
*/
...
else if(srcs.adjoint == 1) /* read adjoint binary format */
   {
   srcs.eqsrc = 0;
   srcs.expl = 0;
   srcs.psrc = 0;
   ...
   mstpar("adjointfile","s",srcs.adjointfile);
   }
...
/* XXXXXX STILL NEED TO TEST */
   ...
   if(srcs.adjoint == 1)
      {
      fprintf(stderr,"**** Initializing adjoint sources:\n");
      fflush(stderr);

      bfilt = 0;  /* hardwire for no filtering */

      get_adjsrc_parP3(&srcs,&rpars_p3);
      init_adjsrcP3(&srcs,&rpars_p3);
...
for(it=0;it<nt;it++)
   ...
   /* exchange velocities */
      ...
      rt1 = times(&tu1);
      blen = exchange1_pvP3(pvfield,pvbuf,&ninfo,it,VEL);
      dtimes(tu1,rt1,&dtu,&drt,&tsize2,blen);

/* XXXXXYYYY */
      if(freesurf && ninfo.minusId_z == -1)
         {
         for(iy=2;iy<ny-1;iy++)
            {
            ...
            fsurf_vel(nx,nz,pvptr,medptr,&fdcoefs,ybndflag);
            }
         }
   ...
   /* exchange stresses */
      ...
      rt1 = times(&tu1);
      blen = exchange1_pvP3(pvfield,pvbuf,&ninfo,it,TAU);
      dtimes(tu1,rt1,&dtu,&drt,&tsize2,blen);
