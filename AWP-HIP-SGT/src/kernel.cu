             ...
             float* dcrjx, float* dcrjy, float* dcrjz, float* d_1, int nyt,   int nzt,   cudaStream_t St, int s_i,   int e_i, int rank)
{
    ...
    dim3 block (VECTOR_LENGTH,1,1);
    dim3 grid ((nzt*nyt-1)/VECTOR_LENGTH+1,1,1);
    ...
    dvelcx<<<grid, block, 0, St>>>(u1, v1, w1, xx, yy, zz, xy, xz, yz, dcrjx, dcrjy, dcrjz, d_1, s_i, e_i,nyt,nzt);
              ...
              cudaStream_t St, int s_j,      int e_j,      int rank,   int rank_me)
{
    ...
    dim3 block (VECTOR_LENGTH,1,1);
    dim3 grid ((nzt*nxt-1)/VECTOR_LENGTH+1,1,1);
    ...
    dvelcy<<<grid, block, 0, St>>>(u1, v1, w1, xx, yy, zz, xy, xz, yz, dcrjx, dcrjy, dcrjz, d_1, s_u1, s_v1, s_w1, s_j, e_j,nxt,nzt);
    err = cudaGetLastError();
                      ...
                      float* b_w1, int nxt,   int nzt,   cudaStream_t St1, cudaStream_t St2, int rank_f,  int rank_b, int rank)
{
    ...
    if(err != cudaSuccess) printf("CUDA KERNEL ERROR! rank=%d update_boundary, before kernel: %s\n",rank,cudaGetErrorString(err));
     ...
     update_boundary_y<<<grid, block, 0, St1>>>(u1, v1, w1, f_u1, f_v1, f_w1, rank_f, Front,nxt,nzt);
     err = cudaGetLastError();
    if(err != cudaSuccess) printf("CUDA KERNEL ERROR! rank=%d update_boundary, intermediate: %s\n",rank,cudaGetErrorString(err));
     ...
     update_boundary_y<<<grid, block, 0, St2>>>(u1, v1, w1, b_u1, b_v1, b_w1, rank_b, Back,nxt,nzt);
    err = cudaGetLastError();
              ...
              int e_i,         int s_j,       int e_j,      int rank, float* p_vx1, float* p_vx2)
{
    ...
    dim3 block (VECTOR_LENGTH,1,1);
    dim3 grid ((nzt*(e_j-s_j+1)-1)/VECTOR_LENGTH+1,1,1);
    ...
    dstrqc<<<grid, block, 0, St>>>(xx,    yy,    zz,  xy,  xz, yz, r1, r2,    r3,    r4,    r5,     r6,
                                   u1,    v1,    w1,  lam, mu, qp, qs, dcrjx, dcrjy, dcrjz, lam_mu, NX,
                                   rankx, ranky, s_i, e_i, s_j, p_vx1, p_vx2,e_j,nzt);
    err = cudaGetLastError();
...
/*
    int nyt=420, nxt=310;
    int i_=2+4*loop,j=2+4*loop,k=awp_align;
    int pos = i_*(nyt+4+8*loop)*(nzt+2*awp_align)+j*(nzt+2*awp_align)+k;
...
/*
    int nyt=420, nxt=310;
    int i=2+4*loop,j=2+4*loop,k=awp_align;
    int pos = i*(nyt+4+8*loop)*(nzt+2*awp_align)+j*(nzt+2*awp_align)+k;
printf("pos=%d\n",pos);
printf("xx,yy,zz,xy,xz,yz=%f,%f,%f,%f,%f,%f\n",
      xx[pos],yy[pos],zz[pos],xy[pos],xz[pos],yz[pos]);
...
{
    register int   i, j, k, pos,     pos_im1, pos_im2, iglob;
    ...
    iglob = blockIdx.x*blockDim.x+threadIdx.x;
    if (iglob>=nzt*nyt)return;
    j=iglob /nzt+2+4*loop;
    k = iglob % nzt+awp_align;
    i    = e_i;
...
__global__ void dvelcy(float* u1,    float* v1,    float* w1,    float* xx,  float* yy,   float* zz,   float* xy, float* xz, float* yz,
                       float* dcrjx, float* dcrjy, float* dcrjz, float* d_1, float* s_u1, float* s_v1, float* s_w1, int s_j,   int e_j, int nxt, int nzt)
{
    register int   i, j, k, pos,     j2,      pos2, pos_jm1, pos_jm2, iglob;
    register int   pos_km2, pos_km1, pos_kp1, pos_kp2;
    ...
    iglob = blockIdx.x*blockDim.x+threadIdx.x;
    if (iglob>=nzt*nxt)return;
    i=iglob /nzt+2+4*loop;
    k = iglob % nzt+awp_align;
    j     = e_j;
    j2    = 4*loop-1;
    pos   = i*d_slice_1+j*d_yline_1+k;
...
__global__ void update_boundary_y(float* u1, float* v1, float* w1, float* s_u1, float* s_v1, float* s_w1, int rank, int flag, int nxt, int nzt)
{
    ...
    //k     = blockIdx.x*BLOCK_SIZE_Z+threadIdx.x+awp_align;
    //i     = blockIdx.y*BLOCK_SIZE_Y+threadIdx.y+2+4*loop;
    register int i, j, k, pos, posj, iglob;
    iglob = blockIdx.x*blockDim.x+threadIdx.x;
    if (iglob>=nzt*nxt)return;
    i=iglob /nzt+2+4*loop;
    k = iglob % nzt+awp_align;
                          ...
                          float* xx,  float* yy,     float* zz,  float* xy,  float* yz,  float* xz)
{
        ...
        idz = psrc[j*dim+2] + awp_align - 1;
        pos = idx*d_slice_1 + idy*d_yline_1 + idz;
        pos_nzt = pos - idz + nzt+awp_align-1;
