...
/*CyberShake change
  Will indicate changes with a comment
  Additionally, many function names are prepended with an underscore
*/
...
if(read_erf == 1)
#ifdef _USE_MEMCACHED
   psrc = _mc_read_ruppars(infile,psrc,&mag,&nstk,&ndip,&dstk,&ddip,&dtop,&avgstk,&avgdip,&elon,&elat,memcached_server);
#else
   psrc = _read_ruppars(infile,psrc,&mag,&nstk,&ndip,&dstk,&ddip,&dtop,&avgstk,&avgdip,&elon,&elat);
#endif
else if(read_gsf == 1)
   psrc = _read_gsfpars(infile,psrc,&gslip,&dstk,&ddip,&dtop,&avgdip);
else
   psrc = _set_ruppars(psrc,&mag,&nstk,&ndip,&dstk,&ddip,&dtop,&avgstk,&avgdip,&rake,&elon,&elat);

printf("Received psrc location %p\n", psrc);
