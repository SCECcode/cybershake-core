...
float orig_avg, orig_sig;
float rtime1_depth, rtime1_depth_range, r_dmin, r_dmax, s_wt, r_wt;
...
RWG 2021-11-16
   ...
   if use_Mw=1 then mag_scale_coef=10.73, i.e., use Mw to convert to moment
            =0 then mag_scale_coef=10.7, i.e., <\bold>M to convert to moment
*/
int use_Mw = 1;
float mag_scale_coef;
...
float ss, dd;
double xx_d, yy_d, zz_d, rr_d;
double ss_d, dd_d, hh_d, ssfac, ddfac, hhfac;
double cosS, sinS, cosD, sinD;
double delta_stk, delta_dip;
...
/* RWG 20191029 - defaults for v5.5.0 */

rtime1_scor = 0.90;

stfparams.beta_depth = 2.0;
stfparams.beta_depth_range = 1.0;
stfparams.beta_deep = 0.13;
stfparams.beta_shal = 0.5;

/* END defaults for v5.5.0 */
...
getpar("print_seed","d",&print_seed);
...
getpar("beta_shal","f",&stfparams.beta_shal);
getpar("beta_deep","f",&stfparams.beta_deep);
getpar("beta_depth","f",&stfparams.beta_depth);
getpar("beta_depth_range","f",&stfparams.beta_depth_range);
...
getpar("fractal_rake","d",&fractal_rake);
...
rtime1_depth = stfparams.beta_depth;
rtime1_depth_range = stfparams.beta_depth_range;
getpar("rtime1_depth","f",&rtime1_depth);
getpar("rtime1_depth_range","f",&rtime1_depth_range);
...
getpar("use_Mw","d",&use_Mw);
mag_scale_coef = 10.73;
if(use_Mw == 0)
   mag_scale_coef = 10.7;
...
getpar("read_slip_file","d",&read_slip_file);
...
bigM = log(10.0);
mom = exp(bigM*1.5*(mag + mag_scale_coef));
...
fprintf(stderr,"\n     slip_c[0]= %14.6e\n\n",sqrt(slip_c[0].re*slip_c[0].re + slip_c[0].im*slip_c[0].im));
...
/*XXXXX
   if(init_slip_file[0] != '\0' && read_slip_file != 0)
      {
	 ...
	 {
         slip_c[ip].re = tsfac1_c[ip].re + slip_c[ip].re;
         slp_avg = slp_avg + slip_c[ip].re;
	 }

      slp_avg = (float)(nstk2*ndip2)/slp_avg;
      for(ip=0;ip<ndip2*nstk2;ip++)
         {
         slip_c[ip].re = slp_avg*slip_c[ip].re;
         slip_c[ip].im = slp_avg*slip_c[ip].im;
	 }
      }
XXXX*/
   ...
   orig_avg = 0.0;
   for(j=0;j<ndip2*nstk2;j++)
      orig_avg = orig_avg + slip_c[j].re;

   orig_avg = orig_avg/(ndip2*nstk2);
   ...
   for(j=0;j<ndip2;j++)
      ...
      for(i=0;i<nstk2;i++)
         {
	 ip = i + j*nstk2;
         orig_sig = orig_sig + (slip_c[ip].re-orig_avg)*(slip_c[ip].re-orig_avg);
         }
      }
   orig_sig = sqrt(orig_sig/(ndip2*nstk2));

fprintf(stderr,"\n     orig_avg= %14.6e orig_sig= %14.6e\n\n",orig_avg,orig_sig);
...
/* RWG 20181009
      ...
      - input values are given in units relative to an average slip of 1.0
      - if read_slip_file=0, then input model is not used
*/

   if(init_slip_file[0] != '\0' && read_slip_file != 0)
      {
      init_slip_IO(tsfac1_c,nstk,ndip,nstk,ndip,&dstk,&ddip,0,init_slip_file);
      ...
      for(ip=0;ip<ndip*nstk;ip++)
         slip_r[ip] = tsfac1_c[ip].re + slip_r[ip];
      }
...
/* check moment and scale slip */
   ...
   fprintf(stderr,"orig_sigma= %f ... ",orig_sig);
...
*/
   ...
   /* XXXXX
    first, if asperities added in, reload slip_c and normalize
    XXXXX */
   if(init_slip_file[0] != '\0' && read_slip_file != 0)
      {
      for(j=0;j<ndip2*nstk2;j++)
      ...
      for(j=0;j<ndip;j++)
	    ...
	    ip = i + j*nstk;
	    ip2 = i + j*nstk2;
            slip_c[ip2].re = slip_r[ip];
            slp_avg = slp_avg + slip_c[ip2].re;
            }
         }
      slp_avg = slp_avg/(ndip2*nstk2);
      ...
      for(j=0;j<ndip2;j++)
         ...
         for(i=0;i<nstk2;i++)
            {
	    ip2 = i + j*nstk2;
            slp_sig = slp_sig + (slip_c[ip2].re-slp_avg)*(slip_c[ip2].re-slp_avg);
            }
         }
      slp_sig = sqrt(slp_sig/(ndip2*nstk2));

      sigfac = orig_sig/slp_sig;
      for(j=0;j<ndip2;j++)
         {
         for(i=0;i<nstk2;i++)
            {
	    ip2 = i + j*nstk2;
            slip_c[ip2].re = sigfac*(slip_c[ip2].re - slp_avg) + orig_avg;
            }
         }
      }
...
/* now do rake */
   ...
   if(fractal_rake)
      {
      hcoef = 1.0;
      kfilt_beta2(rake_c,nstk2,ndip2,&dks2,&dkd2,&hcoef,&lambda_max,&lambda_min,&seed);
      }
   else
      {
      if(use_gaus)
   ...
   for(ip=0;ip<ndip*nstk;ip++)
      ...
      rk_sig = rk_sig + (rake_r[ip]-rk_avg)*(rake_r[ip]-rk_avg);
      }
   rk_sig = sqrt(rk_sig/(ndip*nstk));
...
fprintf(stderr,"\n     tsfac1_c[0]= %14.6e\n\n",sqrt(tsfac1_c[0].re*tsfac1_c[0].re + tsfac1_c[0].im*tsfac1_c[0].im));
...
fprintf(stderr,"\n     rtime1_c[0]= %14.6e\n\n",sqrt(rtime1_c[0].re*rtime1_c[0].re + rtime1_c[0].im*rtime1_c[0].im));
...
/*
   ...
   RWG 20191105:
   Transition to slip array for shallow fault in order to avoid combination of
   very short (~zero) rise time plus appreciable slip (=> very high acceleration).
*/

   r_dmin = rtime1_depth - rtime1_depth_range;
   r_dmax = rtime1_depth + rtime1_depth_range;

   rt1_avg = 0.0;
   for(j=0;j<ndip;j++)
      {
      if(psrc[j*nstk].dep <= r_dmin)
         {
         s_wt = 1.0;
         r_wt = 0.0;
         }
      else if(psrc[j*nstk].dep < r_dmax && psrc[j*nstk].dep > r_dmin)
         {
         r_wt = (psrc[j*nstk].dep - r_dmin)/(r_dmax-r_dmin);
         s_wt = 1.0 - r_wt;
         }
      else
         {
      ...
      for(i=0;i<nstk;i++)
         ...
         rtime1_r[ip] = r_wt*rtime1_c[ip2].re + s_wt*slip_c[ip2].re;
 ...
 * normal vector points from footwall towards hanging wall *
...
*/
         ...
         ss_d = ssfac*(rough_c[ip2+1].re - rough_c[ip2-1].re);            /* df/ds */
         dd_d = ddfac*(rough_c[ip2+nstk3].re - rough_c[ip2-nstk3].re);    /* df/dd */
         hh_d = -1.0;

         hhfac = 1.0/sqrt(ss_d*ss_d + dd_d*dd_d + hh_d*hh_d);
...
/* now, rotate unit normal into N,E,Z coordinates-
   ...
   Also compute "rr", the normal vector component in the horizontal plane perpendicular
   to original strike (ss) direction. If this component goes negative, then the perturbed
   fault surface has "rolled over" with respect to the ground surface. This can happen
   when the original dip is small and/or the perturbations are large. In this case, the
   computed strike direction flips with respect to the original strike. However, the sense
   of fault slip will now be in the opposite direction from the original because the rake
   has not changed. There are two possible solutions to this:

      1) retain the computed strike & dip, but then add 180 to the rake for this subfault
      2) retain original rake, add 180 to the computed strike and change the sign of
         computed dip

   I implement 2) because it keeps all the subfault strikes pointing in roughly the
   same direction, and it keeps the original rake which hopefully avoids confusion.
   But this means the dip can be negative (which is fine mathematically, see below).

   NOTE: for moment-tensor computations, the results are unchanged under the following
   transformations

      1) stk'=stk0-180, dip'=180-dip0, rak'=-rak0
      2) stk'=stk0-180, dip'=-dip0, rak'=rak0-180

   The above relations can be used to confirm the validity of the A&R convention
   even when the dip is negative or the dip is greater than 90.

*/

         rr_d = dd_d*cosD - hh_d*sinD;

         xx_d = ss_d*cosS - rr_d*sinS;
         yy_d = ss_d*sinS + rr_d*cosS;
         zz_d = dd_d*sinD + hh_d*cosD;
...
*/
	 ...
	 psrc[ip].stk = atan2(yy_d,xx_d)/rperd - 90.0;
	 psrc[ip].dip = 180.0 - acos(zz_d)/rperd;

	 if(rr_d < 0.0) /* check if fault has "rolled over" */
	    {
	    psrc[ip].stk = psrc[ip].stk - 180.0;
	    psrc[ip].dip = -psrc[ip].dip;
	    }
