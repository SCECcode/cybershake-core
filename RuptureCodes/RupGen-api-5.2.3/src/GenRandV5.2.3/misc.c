...
//printf("Allocated %d bytes to location %p.\n", len, ptr);
...
//printf("Reallocated %d bytes to location %p.\n", len, ptr);
return(ptr);
}
...
*shyp = 0.5*(*flen)*_sfrand(seed);
*dhyp = 0.5*(*fwid)*(1.0 + _sfrand(seed));
}
