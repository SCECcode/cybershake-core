...
#else
   psrc_orig = _read_ruppars(infile,psrc,&mag,&nstk,&ndip,&dstk,&ddip,&dtop,&avgstk,&avgdip,&elon,&elat);
#endif
else if(read_gsf == 1)
   psrc_orig = _read_gsfpars(infile,psrc_orig,&gslip,&dstk,&ddip,&dtop,&avgdip);
else
   psrc_orig = _set_ruppars(psrc_orig,&mag,&nstk,&ndip,&dstk,&ddip,&dtop,&avgstk,&avgdip,&rake,&elon,&elat);
