...
************************************************************************************
   ...
   * Changed default/recommended slip rate function to 'Mliu' which is modified version
     of UCSB function.  See _gen_Mliu_stf() in srf_subs.c for details.
...
nstk2 = (int)(extend_fac*nstk);
if(nstk2%2)
   nstk2++;
ndip2 = (int)(extend_fac*ndip);
if(ndip2%2)
   ndip2++;
...
nstk3 = 3*nstk;
nstk3 = (int)(extend_fac*nstk3);
if(nstk3%2)
   nstk3++;

ndip3 = 3*ndip;
ndip3 = (int)(extend_fac*ndip3);
if(ndip3%2)
   ndip3++;
...
if(fdrup_time == 1)
   ...
   fdrt = (double *)check_malloc(ntot*sizeof(double));
   rslw = (double *)check_malloc(ntot*sizeof(double));
   rspd = (float *)check_malloc(ntot*sizeof(float));

   fspace = (double *)check_malloc((nstk+ndip)*sizeof(double));
   ispace = (int *)check_malloc((nstk+ndip)*sizeof(int));
   }
...
/* now do roughness */
   ...
   hcoef = 1.0;
   kfilt_beta2(rough_c,nstk3,ndip3,&dks3,&dkd3,&hcoef,&flen,&lambda_min,&seed);
...
if(tsfac_rand <= 0.0)
   ...
   hcoef = 1.0;
   kfilt_beta2(tsfac2_c,nstk3,ndip3,&dks3,&dkd3,&hcoef,&flen,&lambda_min,&seed);
...
if(rtime_rand <= 0.0)
   ...
   hcoef = 1.0;
   kfilt_beta2(rtime2_c,nstk3,ndip3,&dks3,&dkd3,&hcoef,&flen,&lambda_min,&seed);
...
/* XXXX 2014/11/20 new formulation for strike and dip variability */
   if(sd_rand > 0.0)
      {
      }
...
/* 20140422 RWG: Deep rupture speed decrease */
      ...
      if(fdrup_time == 1)
         ...
         get_rslow(rspd,rslw,nstk,ndip,&rvel_rand,&seed);

	 ixs = (int)((shypo+0.5*flen)/dstk + 0.5);
	 iys = (int)(dhypo/ddip + 0.5);
	 wfront2d_(&nstk,&ndip,&ixs,&iys,&dh,&nsring,fdrt,rslw,&ntot,fspace,ispace);
	 }
      else
