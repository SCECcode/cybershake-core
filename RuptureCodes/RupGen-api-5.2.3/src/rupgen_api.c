...
int rupgen_genslip_with_params(char* rup_geom_file, int slip, int hypo, rg_stats_t *stats, struct standrupformat* srf, int hypo_dist, float dt, char* params) {
	...
	int index = 0;
        ...
        //Arguments for v5.2.3a
        sprintf(rgargv[9], "fdrup_time=1");
        sprintf(rgargv[10], "srf_version=2.0");
        sprintf(rgargv[11], "alpha_rough=0.01");
	...
	while (tok!=NULL) {
        ...
        //Round flen, fwid, dtop, shyp, dhyp to 2 places
	//v5.2.3a: Round slip1, slip2, slip3 to 4 places
