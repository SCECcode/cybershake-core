...
//seis contains the seismogram data
//nt is the number of timesteps
//dt is the size of the timesteps
//rot is the rotation angle we want the X component to be at after rotation
//(note that this is NOT the same as the amount of rotation to apply)
...
shead1->nt = nt;
shead1->dt = dt;
sprintf(shead1->comp, "0.0");
shead2->nt = nt;
shead2->dt = dt;
sprintf(shead2->comp,"90.0");

shead2->hr = shead1->hr = 0;
shead2->min = shead1->min = 0;
shead2->sec = shead1->sec = 0;
