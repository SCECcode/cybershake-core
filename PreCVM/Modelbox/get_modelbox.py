...
def opendb(h,pt,us,pwd,d,sqlite=False):
  if sqlite==False:
    try:
      ...
      return connection
    except pymysql.Error as message:
...
if len(sys.argv) < 7:
  print("Syntax: get_modelbox.py SITE_Name ERF_ID Outfile_name spacing(in km) Server rotation [padding] ['gpu'] ['bbox'] ['tight']")
...
gpu = False
if len(sys.argv) >= 9 and "gpu" in sys.argv[8:]:
	print("GPU mode.")
	gpu = True

bbox = False
if len(sys.argv) >= 9 and "bbox" in sys.argv[8:]:
	print("Bounding box mode.")
	bbox = True

tight = False
if len(sys.argv) >= 9 and "tight" in sys.argv[8:]:
	...
	#Want a rule so we use (in AWP coords) 10x10x1 for 0.5 Hz/200m meshes, 20x10x1 for 1Hz/175m meshes, 40x20x1 for 1 Hz/100m meshes; will use spacing for now
        ...
        if spacing==0.2:
	        ...
	        #Changed to 40x30x1 for Study 18.8, since some volumes are large and 300 sec
                #Try 20x20x1 for Summit RSQSim testing
                #xlrnd += spacing*((-1*nx) % 60)
                #ylrnd += spacing*((-1*ny) % 80)
                xlrnd += spacing*((-1*nx) % 40)
                ylrnd += spacing*((-1*ny) % 40)
        elif spacing==0.025:
	        xlrnd += spacing*((-1*nx) % 160)
	        ylrnd += spacing*((-1*ny) % 160)
        elif math.fabs(spacing-0.0333)<0.001:
	        xlrnd += spacing*((-1*nx) % 80)
	        ylrnd += spacing*((-1*ny) % 80)
                #xlrnd = spacing*3840
                #ylrnd = spacing*3840
        else:
