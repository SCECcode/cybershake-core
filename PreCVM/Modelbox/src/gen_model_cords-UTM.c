...
if(do_coords)
   ...
   flon = (float *) check_malloc (gp.nx*gp.ny*sizeof(float));
   flat = (float *) check_malloc (gp.nx*gp.ny*sizeof(float));
   ...
   for(iy=0;iy<gp.ny;iy++)
      ...
      for(ix=0;ix<gp.nx;ix++)
         ...
         dxr = xr0 + 1000.0*((gp.xp[ix])*ar1 - (gp.yp[iy])*ar2);
         dyr = yr0 - 1000.0*((gp.xp[ix])*ar4 + (gp.yp[iy])*ar3);
         geoutm_(&dlon,&dlat,&dxr,&dyr,&utm_zone,&utm2geo);
