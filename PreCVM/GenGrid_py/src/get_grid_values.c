...
//This code is modified from get_model_cords.c to return (X,Y) grid coordinates for arbitrary (lat, lon)
...
*/
float lat, lon, grid_spacing;
char infile[1024];
mstpar("infile","s",infile);
mstpar("outfile","s",outfile);
mstpar("grid_spacing_km","f",&grid_spacing);
...
//   {
   ...
   //for(iy=0;iy<gp.ny;iy++)
	...
	//Iterate over infile, converting each point
