...
int ucvm_grid_gen_private(ucvm_proj_t *iproj, ucvm_trans_t *trans,
			  ucvm_proj_t *oproj,
...
int ucvm_grid_convert_private(ucvm_proj_t *iproj, 
			      ucvm_proj_t *oproj, 
			      ...
			      const char *filename)
{
  ...
  if (strcmp(iproj->proj, "cmu") == 0) {
    ...
    if (pnts != NULL) {
      for (i = 0; i < n; i++) {
	bilinear_xy2geo(&cmu, pnts[i].coord[0], pnts[i].coord[1], &x, &y);
	pnts[i].coord[0] = x;
	pnts[i].coord[1] = y;
      }
    }
	...
	if (num_buffered > 0) {
	  for (i = 0; i < num_buffered; i++) {
	    /* Translate point */
	    bilinear_xy2geo(&cmu, pbuf[i].coord[0], pbuf[i].coord[1], &x, &y);
	    pbuf[i].coord[0] = x;
	    pbuf[i].coord[1] = y;
	  }
    ...
    strcpy(ipdesc, "+proj=latlong +datum=WGS84");
    ipj = pj_init_plus(ipdesc);
  } else {
    strcpy(ipdesc, iproj->proj);
    ipj = pj_init_plus(iproj->proj);
  }
  ...
  if (strcmp(oproj->proj, "cmu") == 0) {
    strcpy(opdesc, "+proj=latlong +datum=WGS84");
    opj = pj_init_plus(opdesc);
  } else {
    strcpy(opdesc, oproj->proj);
    opj = pj_init_plus(oproj->proj);
  }
  ...
  if (pnts != NULL) {
	...
	pnts[i].coord[1] = pnts[i].coord[1] * RAD_TO_DEG;
      ...
      if (strcmp(oproj->proj, "cmu") == 0) {
	bilinear_geo2xy(&cmu, pnts[i].coord[0], pnts[i].coord[1], &x, &y);
	pnts[i].coord[0] = x;
	pnts[i].coord[1] = y;
      }
  ...
  if (filename != NULL) {
	...
	for (i = 0; i < num_buffered; i++) {
	  ...
	  if (strcmp(oproj->proj, "cmu") == 0) {
	    bilinear_geo2xy(&cmu, pbuf[i].coord[0], pbuf[i].coord[1], &x, &y);
	    pbuf[i].coord[0] = x;
	    pbuf[i].coord[1] = y;
	  }
	}
