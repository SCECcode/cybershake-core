...
def run_det_surf_model(gridout, coords, models):
	...
	cmd = "/bin/date; aprun -n %d -N 8 -S 4 %s/UCVM/bin/determine_surface_model_mpi %s %s %s %s" % (np, CS_PATH, gridout, coords, models, output_file)
	print cmd
...
def run_det_smooth_pts(surf_file, coords, nx, ny, smooth_dist):
	...
	cmd = "/bin/date; aprun -n 1 %s/UCVM/smoothing/determine_smoothing_points.py %s %s %d %d %s %s" % (CS_PATH, surf_file, coords, nx, ny, smooth_dist, output_file)
	print cmd
	exitcode = os.system(cmd)
	if (exitcode!=0):
...
def run_smooth(input_mesh, smooth_pts, nx, ny, nz, smooth_dist, output_mesh):
	#Set up stroping first
	cmd = "%s setstripe -c 100 -s 5m %s" % (LFS_CMD, output_mesh)
	os.system(cmd)
	...
	np = min(nz, np)
        MPI_CMD = "%s -n %d -N 8 -S 4" % (MPI_CMD, np)
	cmd = "/bin/date; %s %s/UCVM/smoothing/smooth_mpi %s %s %d %d %d %s %s" % (MPI_CMD, CS_PATH, input_mesh, smooth_pts, nx, ny, nz, smooth_dist, output_mesh)
	print cmd
	exitcode = os.system(cmd)
	if (exitcode!=0):
...
os.environ["LD_LIBRARY_PATH"] = "/lustre/atlas/proj-shared/geo112/CyberShake/software/UCVM/ucvm-18.5.0/lib/euclid3/lib:/lustre/atlas/proj-shared/geo112/CyberShake/software/UCVM/ucvm-18.5.0/lib/proj-4/lib:/lustre/atlas/proj-shared/geo112/CyberShake/software/UCVM/ucvm-18.5.0/model/cvms426/lib:/lustre/atlas/proj-shared/geo112/CyberShake/software/UCVM/ucvm-18.5.0/model/cencal/lib:/lustre/atlas/proj-shared/geo112/CyberShake/software/UCVM/ucvm-18.5.0/model/cvms5/lib:/lustre/atlas/proj-shared/geo112/CyberShake/software/UCVM/ucvm-18.5.0/model/cca/lib:%s" % (os.environ["LD_LIBRARY_PATH"])
