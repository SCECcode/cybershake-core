...
C  Defaults for CyberShake constants
      sdrop = 50
      rayset(1) = 2
      rayset(2) = 1
      rayset(3) = 2
...
C  Derive rvfac from other parameters, as given in hfsims_cfg.py in BBP
C  This only works if rand_num=2, because we know 0.9560342718892494
C  is what comes out of python 2.7.5 random(), with seed of 2, on BW.
      if(rand_num.ne.2) then
	write(*,*) "rand_num must be 2 to derive rvfac, aborting."
	stop 1
      endif
...
c      print*,'stress_average= ',stress_average
...
c      read(5,*) rvfac,shal_rvfac,deep_rvfac,fcfac,Calpha
...
c      rhosig = -1.0
c      qssig = -1.0
c      read(5,*) nlskip,vpsig,vshsig,rhosig,qssig,icflag
c      read(5,'(a256)') velname                          
      ...
      fasig1 = -1.0
      fasig2 = -1.0
c      read(5,*) fasig1,fasig2,rvsig1

CCC RWG modified 2014-09-19
c      read(5,*) ipdur_model
...
c      do
c         read(1,*) dummy
c         !This line checks to see if input starts with a '#' or '%'
c         !If so, ignore
c         if ((dummy(1:1) .ne. '#') .AND. (dummy(1:1) .ne. '%')) exit
c         head_lines=head_lines+1
c      enddo
