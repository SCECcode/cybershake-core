...
c     Subroutine for generating lookup table for FAST

      subroutine gen_lookup_table()

      include 'params.h'

      integer KMAX,kmax_index,max_kmax,max_kmax_index

      complex*8 lookup_table(mm,16,2)
      complex*8 THETA,CTHETA
      integer IND

      common /fastconst/ lookup_table

      write(*,*) 'generating lookup table'

      max_kmax = mm
      max_kmax_index = int(log(float(max_kmax))/log(2.0))

      if(max_kmax_index.gt.16) then
        write(*,*) 'kmax can be bigger than 2^16. Increase size of lookup table.'
        stop
      endif

      do 3 IND=-1,1,2
        do 2 kmax_index=0,max_kmax_index
          KMAX=2**kmax_index
          do 1 K=1,KMAX
            THETA=cmplx(0.0,3.141593*FLOAT(IND*(K-1))/FLOAT(KMAX))
            CTHETA=CEXP(THETA)
            lookup_table(K,kmax_index,(IND+1)/2+1) = CTHETA
1         continue
2       continue
3     continue
...
c     Subroutine for initializing parameters to the right CS values
      subroutine cs_params()
     ...
     +spar_exp
      ...
      subroutine hb_high(cap, stlon, stlat, velfile, outname, duration, dt, ds, nevnt,
...
CFFF This replaces ntopq in even_dist1
      ...
      CHARACTER*256 outname,velfile
      CHARACTER*256 velname,velname1
      character*64 cap

      integer head_lines, j, dosetup
      integer nlskip,icflag,msite

      integer isite_amp,nbu,iftt,nrtyp
      integer irand,nsite,ipdur_model,ispar_adjust,ndata
      real*4 targ_mag,fault_area,vr
      real*4 flol,fhil,duration,fmx,akapp,fasig1,fasig2,rvsig1
      real*4 sm,stress_average,rvfmax,dris,bigC

      real*4 e,ratim_delta
...
c Enables or disables debugging print statements
      debug = 0
...
C Direct computation of bigC using fce_avg and fcmain: bigC = (fce_avg/fcmain)^2
      ...
      if(debug.eq.1) then
        print*,'fce_avg=',fce_avg,' fcmain=',fcmain
      endif
...
c      A3=EXP(-OMG*Rxx/(2.0*QV*BETA))/Rxx                                       

c   use input q values via qbar
c      A3=EXP(-0.5*OMG*qb*(fr**(-0.5)))/Rxx
...
C     COMPLEX*8 ACE(30000),TEMP,THETA                                            
      ...
      complex*8 lookup_table(mm,16,2) 
      COMPLEX*8 ACE(1),TEMP,THETA,CTHETA                                            
      integer kmax_index,ind_index
  ...
  140 IF(KMAX.GE.NNN) RETURN                                                    
      ISTEP=KMAX*2
c     determine kmax_index
      
      if(2**kmax_index.ne.KMAX) then
        write(*,*) 'ERROR: 2^',kmax_index,'!=',KMAX
      endif
      DO 150 K=1,KMAX
c      write(*,*) 'kmax=',kmax                                                           
c      THETA=CMPLX(0.0,3.141593*FLOAT(IND*(K-1))/FLOAT(KMAX))
       CTHETA=lookup_table(K,kmax_index,ind_index)                    
      DO 160 I=K,NNN,ISTEP                                                      
      J=I+KMAX                                                                  
c      TEMP=ACE(J)*CEXP(THETA)                                                   
      TEMP=ACE(J)*CTHETA
      ACE(J)=ACE(I)-TEMP                                                        
      ACE(I)=ACE(I)+TEMP                                                        
  160 CONTINUE                                                                  
...
c      print*,'hs,ksrc,j,j0,kbot= ',hs,ksrc,j,j0,kbot
