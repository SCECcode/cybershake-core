...
def genFaultList(outputName, site, erf_id):
    ...
    command = 'export PATH=/share/apps/pgi/7.1/linux86/7.1-2/jre/bin:$PATH;java -classpath .:%s:%s/faultlist/mysql-connector-java-5.0.5-bin.jar faultlist/CreateFaultList %s %s %s %s' % (sys.path[0], sys.path[0], site, erf_id, PATH_TO_RUPTURE_VARIATIONS, outputName)
#    command = 'java -classpath .:%s:%s/faultlist/mysql-connector-java-5.0.5-bin.jar faultlist/CreateFaultList %s %s %s %s' % (sys.path[0], sys.path[0], site, erf_id, PATH_TO_RUPTURE_VARIATIONS, outputName)
    print "genFaultList cmd: " + command
#    print command
...
def genSgtGrid(outputFile, site, ns, src, mlon, mlat, mrot, faultlist, radiusfile):
	...
	modules = 'export MODULEPATH=/opt/modulefiles:/opt/apps/teragrid/modulefiles:/opt/apps/modulefiles; export MODULESHOME=/opt/modules; module load TACC'
	command = '%s; %s %s/bin/gen_sgtgrid nx=%d ny=%d nz=%d h=%f xsrc=%d ysrc=%d ixmin=%d ixmax=%d iymin=%d iymax=%d izstart=%d izmax=%d radiusfile=%s outfile=%s modellon=%f modellat=%f modelrot=%f faultlist=%s' % (modules, MPI_CMD, sys.path[0], ns[0], ns[1], ns[2], HH, src[0], src[1], IX_MIN, IX_MAX, IY_MIN, IY_MAX, IZ_START, IZ_MAX, radiusfile, outputFile, mlon, mlat, mrot, faultlist)
