...
/*if my_id>N:
	...
	exit
*/
void do_work(int argc, char** argv, struct sgtmaster* sgtmast, struct sgtindex* sgtindx, struct sgtfileparams* sgtfilepar, int* proc_points, int num_sgt_handlers, char stat[64], float slat, float slon, int run_id, float det_max_freq, float stoch_max_freq, int run_PSA, int run_rotd, int my_id);
...
int worker(int argc, char** argv, int num_sgt_handlers, struct sgtfileparams* sgtfilepar, char stat[64], float slat, float slon, int run_id, float det_max_freq, float stoch_max_freq, int run_PSA, int run_rotd, int my_id) {
	...
	do_work(argc, argv, &sgtmast, sgtindx, sgtfilepar, proc_points, num_sgt_handlers, stat, slat, slon, run_id, det_max_freq, stoch_max_freq, run_PSA, run_rotd, my_id);
...
void do_work(int argc, char** argv, struct sgtmaster* sgtmast, struct sgtindex* sgtindx, struct sgtfileparams* sgtfilepar, int* proc_points, int num_sgt_handlers, char stat[64],
	...
	while (1) {
		...
		if (msg.msg_type==WORK_RESPONSE) {
			...
			t_info.argc = argc;
			t_info.argv = argv;
			run_synth(&t_info, proc_points, num_sgt_handlers, stat, slat, slon, run_id, det_max_freq, stoch_max_freq, run_PSA, run_rotd, my_id);
