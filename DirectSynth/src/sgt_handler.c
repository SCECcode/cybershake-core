...
int sgt_handler(struct sgtfileparams* sgtfilepar, int num_comps, MPI_Comm* sgt_handler_comm, int num_sgt_handlers, int my_id) {
	...
	//Listen for messages
	handler_listen(sgtmast, sgtindx, sgthead, sgtdata, my_id);

	return 0;
}


void handler_listen(struct sgtmaster* sgtmast, struct sgtindex* sgtindx, struct sgtheader* sgthead[3], char** sgtdata, int num_my_points, int my_id, int num_comps) {
	/* Listen for messages:
			if request for data:
				service request for SGT data
			if work complete from process N:
	...
	*/
	...
	while (1) {
		...
		if (msg.msg_type==SGT_REQUEST) {
			if (debug) {
				char buf[256];
				sprintf("Received request for %d SGTs from process %d.", msg.num_sgts_requested, msg.msg_src);
				write_log(buf);
			}
...
void handle_SGT_request(long long* sgts_requested, int num_sgts_requested, cfuhash_table_t* hash_table, struct sgtmaster* sgtmast, struct sgtheader* sgthead[3],
	...
	for (i=0; i<num_sgts_requested; i++) {
		...
		int found = cfuhash_get_data(hash_table, &(sgts_requested[i]), key_len, &(sgts_to_send_index_list[i]), val_len);
		if (!found) {
	...
	*sgtdata = check_malloc(3*sizeof(char*));
	...
	for (i=0; i<3; i++) {
		if (sgt_files[i][0] != '\0') {
			if (debug) {
				char buf[256];
				sprintf(buf, "Reading from SGT file %s.", sgt_files[i][0]);
				write_log(buf);
			}
			readSGT_MPI(sgtfilepar.xfile, sgtmast.globnp, num_my_points, sgtmast.nt, proc_points[my_id], (*sgtdata)[i], separate_header_flag[0], sgt_handler_comm);
		}
	}
}
